        ...
        if st.button("🎨 Aperçu simple", type="secondary"):
            ...
            for month in range(1, 13):
                ...
                if not month_data.empty:
                    ...
                    # Créer une ligne de couleurs pour le mois
                    colors_html = ""
                    for _, row in month_data.iterrows():
                        color, _ = get_color_for_temperature(row['temperature'], palette_info['colors'])
                        colors_html += f'<span style="background-color: {color}; width: 10px; height: 20px; display: inline-block; margin: 1px;" title="{row["temperature"]}°C"></span>'
                    
                    st.markdown(f'<div style="line-height: 25px;">{colors_html}</div>', unsafe_allow_html=True)
