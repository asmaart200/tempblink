...
def page_configuration():
    ...
    with col2:
        ...
        # Créer l'affichage de la palette avec Plotly
        colors_display = []
        labels_display = []
        ...
        for i, (color, yarn_name) in enumerate(zip(palette_info['colors'], palette_info['yarn_colors'])):
            ...
            labels_display.append(f"{yarn_name} - {temp_range}")
        
        # Créer un graphique en barres horizontal pour la palette
        fig_palette = go.Figure()
        fig_palette.add_trace(go.Bar(
            y=labels_display,
            x=[1] * len(colors_display),
            orientation='h',
            marker=dict(color=colors_display),
            showlegend=False,
            text=labels_display,
            textposition='inside',
